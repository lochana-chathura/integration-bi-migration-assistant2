...
public class BallerinaVisitor extends DataWeaveBaseVisitor<Void> {
    ...
    @Override
    public Void visitArrayType(DataWeaveParser.ArrayTypeContext ctx) {
        this.dwContext.append("Array<");
    ...
    @Override
    public Void visitConditionalExpressionWrapper(DataWeaveParser.ConditionalExpressionWrapperContext ctx) {
    ...
    @Override
    public Void visitPayloadExpression(DataWeaveParser.PayloadExpressionContext ctx) {
        this.dwContext.currentScriptContext.currentType = DWUtils.PAYLOAD; // TODO
        this.dwContext.append("payload");
        stats.record(DWConstruct.IDENTIFIER, true);
        return null;
    }
    ...
    public Void visitWhenCondition(DataWeaveParser.WhenConditionContext ctx) {
        ...
        if (size < 1) {
            throw new RuntimeException("Invalid when condition structure");
        }
        ...
        visit(ctx.operationExpression());
        String firstResult = dwContext.getExpression();

        // First condition
        visit(conditions.get(0));
        builder.setIfCondition(new Expression.BallerinaExpression(dwContext.getExpression()));
        builder.addIfBody(new BallerinaStatement(varName + " = " + firstResult + ";"));

        // Process middle when-otherwise pairs as else-if clauses
        for (int i = 1; i < size - 1; i += 2) {
            if (i + 1 < size) {
                visit(conditions.get(i + 1)); // Next condition
                String condition = dwContext.getExpression();
                visit(conditions.get(i)); // Result for this condition
                List<Statement> statements = new ArrayList<>();
                statements.add(new BallerinaStatement(varName + " = " + dwContext.getExpression() + ";"));
